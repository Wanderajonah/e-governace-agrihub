...
const generateReportData = async (type, period) => {
  ...
  const [
    totalFarmers, newFarmers, totalProduce, verifiedProduce,
    pendingVerification, transactionData, priceData,
  ] = await Promise.all([
  ...
  return {
    period: { start, end: end || start, label: period || type },
    summary: {
      totalFarmers, newFarmers, totalProduce, verifiedProduce, pendingVerification,
...
const listReports = asyncHandler(async (req, res) => {
  ...
  const [reports, total] = await Promise.all([
    Report.find(filter).populate('generatedBy', 'name email')
      .sort(sortOption).skip(skip).limit(pageLimit),
    Report.countDocuments(filter),
  ]);

  return paginatedResponse(res, reports, total, currentPage, pageLimit);
});

module.exports = { generateReport, getReport, listReports };
