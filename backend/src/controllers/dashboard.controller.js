...
const getStats = asyncHandler(async (req, res) => {
  ...
  const [
    totalFarmers, registeredProduce, verifiedProduce, pendingVerification,
    todayTransactionsResult, monthlyRevenue, avgPrices, recentActivities, unreadNotifications,
  ] = await Promise.all([
    ...
    Transaction.aggregate([
      ...
      { $limit: 12 },
      {
        $project: {
          ...
          revenue: { $round: ['$revenue', 2] }, count: 1,
        },
      },
    ]),
    CommodityPrice.aggregate([
      {
        $group: {
          _id: '$commodity', avgPrice: { $avg: '$price' },
          lastPrice: { $last: '$price' }, unit: { $first: '$unit' },
        },
      },
      {
        $project: {
          ...
          lastPrice: { $round: ['$lastPrice', 2] }, unit: 1,
        },
      },
      { $sort: { commodity: 1 } },
    ]),
  ...
  const stats = {
    ...
    monthlyRevenue, avgPrices, recentActivities, unreadNotifications,
  };
